...
class CatsRepositoryImpl(
  ...
  override fun observeCats(): Observable<List<Cat>> = catsObservable
    .doOnNext { cats -> if (cats.isEmpty()) getCatsFromRemote(1, 20).subscribe() }
  ...
  private fun getCatsFromRemote(page: Int, limit: Int) =
    remoteSourceCats.getNextCatsPage(page, limit).switchMapCompletable { localSourceCats.insertCats(it) }
  ...
  override fun insertCats(people: List<Cat>): Completable =
    Completable.fromAction { localSourceCats.insertCats(people) }
