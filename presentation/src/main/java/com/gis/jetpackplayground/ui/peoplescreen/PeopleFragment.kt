...
import com.gis.jetpackplayground.databinding.FragmentPeopleBinding
import com.gis.jetpackplayground.ui.peoplescreen.PeopleIntent.*
...
class PeopleFragment : Fragment() {
  ...
  private fun initRecyclerView(context: Context) {
    val adapter = PeopleAdapter(itemMovedPublisher, itemDeletedPublisher)
    binding.rvPeople
      .apply {
        layoutManager = LinearLayoutManager(context, RecyclerView.VERTICAL, false)
        setAdapter(adapter)
      }.also {
        val itemTouchHelper = ItemTouchHelper(CustomItemTouchHelperCallback(adapter))
  ...
  private fun initIntents() {
    val intents = Observable.merge(listOf(
  ...
  private fun render(state: PeopleState) {
    srlRefreshPeople.isRefreshing = state.loading

    if (state.error != null)
      Snackbar.make(view!!, state.error.message!!, Snackbar.LENGTH_SHORT).show()

    (rvPeople.adapter as PeopleAdapter).submitList(state.people)
  }
}